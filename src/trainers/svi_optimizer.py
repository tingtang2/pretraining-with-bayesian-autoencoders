# code from https://github.com/harvardnlp/sa-vae/blob/master/optim_n2n.py
...
class OptimN2N:
    ...
    def __init__(self,
        ...
        if self.acc_param_grads:
            ...
            self.param_grads = [
                torch.zeros([self.iters] + list(p.size())).type_as(p.data)
    ...
    def grad_norm(self, g_list, g_norm_list):
        for g, g_norm in zip(g_list, g_norm_list):
            ...
            g.div_(g_norm2.unsqueeze(1).expand_as(g)).mul_(
                g_norm.unsqueeze(1).expand_as(g))
    ...
    def forward_mom(self, input, y, verbose=False):
        ...
        for k in range(self.iters):
            self.all_z.append(
                Variable(
                    torch.cuda.FloatTensor(input[0].size()).normal_(0, 1)))
            ...
            all_grads_k = torch.autograd.grad(loss,
                                              all_input_params,
                                              retain_graph=True)
    ...
    def backward_mom(self, grad_output, verbose=False):
        input_kp1_grad = [g for g in grad_output]
        ...
        for k in reversed(range(rev_iters)):
            ...
            for i in range(len(p_kp1_grad)):
                ...
                assert x_k_rv.requires_grad == True
            ...
            all_grads_rv_k = torch.autograd.grad(loss,
                                                 all_input_params,
                                                 retain_graph=True)
